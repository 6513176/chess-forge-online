...
const mkUid = () => Math.random().toString(36).slice(2) + Date.now().toString(36);
const makeCard = (id: CardId): CardInstance => ({ ...CARD_MAP[id], uid: mkUid() });
...
export default function RoomPage() {
  ...
  const [shield, setShield] = useState<{by:null|'w'|'b', square:null|string}>({ by:null, square:null });
  ...
  // แจกการ์ดเริ่มต้น
  useEffect(() => {
    setHand([makeCard('BUFF_EXTRA_MOVE'), makeCard('DEF_SHIELD'), makeCard('COUNTER_SACRIFICE')]);
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await joinRoom(String(roomId));
      if (!mounted) return;
      if (!res.ok) { alert('เข้าห้องไม่ได้: ' + (res.reason ?? 'unknown')); return; }
      ...
      const onAward = ({ id }: { id: CardId }) => {
        setHand(h => [...h, makeCard(id)]);
      };
  ...
  function sendChat() {
    ...
    socket.emit('chat:message', { roomId, text: message, from: color ?? 'spectator' });
    setMessage('');
  }
  ...
  function handleMove({ from, to }: { from: Square; to: Square }) {
    ...
    const willPromote =
      p.type === 'p' && ((p.color === 'w' && to[1] === '8') || (p.color === 'b' && to[1] === '1'));
    const moveObj: any = { from, to };
    if (willPromote) moveObj.promotion = 'q';

    const fenBefore = game.fen();
    let mv: any;
    try { mv = game.move(moveObj); } catch { return false; }
    if (!mv) return false;
  ...
  function playCard(card: CardInstance) {
    ...
    socket.emit('card:play', { roomId, color, card: card.id }, (ack:any) => {
      if (ack?.ok) {
        setHand(h => {
          const cp = h.slice();
          const i = cp.findIndex(x => x.uid === card.uid);
  ...
  function handleSquareClickForShield(square: Square) {
    ...
    socket.emit('card:play', { roomId, color, card: 'DEF_SHIELD', payload: { square } }, (ack:any) => {
      if (ack?.ok) {
        setHand(h => {
          const cp = h.slice();
          const i = cp.findIndex(x => x.uid === shieldCardUid);
          if (i >= 0) cp.splice(i, 1);
          return cp;
        });
      } else {
  ...
  function handleSquareClickForCounter(square: Square) {
    ...
    const p:any = gameRef.current.get(square as any);
    if (!p || p.color !== meSide || p.type === 'k') {
      alert('เลือกเฉพาะหมากของคุณ (ยกเว้นราชา) เพื่อสละชีพ');
      return;
    }
    socket.emit('card:play', {
      roomId, color, card: 'COUNTER_SACRIFICE', payload: { sacrificeSquare: square }
    }, (ack:any) => {
      if (ack?.ok) {
        setHand(h => {
          const cp = h.slice();
          const i = cp.findIndex(x => x.uid === counterCardUid);
          if (i >= 0) cp.splice(i, 1);
          return cp;
        });
      } else {
    ...
    <div className="min-h-screen p-4 md:p-8 grid gap-4 md:grid-cols-[1fr_320px]">
      ...
      <div className="bg-gray-900 rounded-2xl p-4 text-white flex flex-col gap-3">
        <div className="flex justify-between items-center">
          <div>Room: <span className="font-mono">{String(roomId)}</span></div>
          <div>คุณคือ: <b>{color ?? 'กำลังเข้าห้อง…'}</b> | เทิร์น: <b>{turn === 'w' ? 'white' : 'black'}</b></div>
        </div>
        ...
        <div className="self-center" style={{ width: 560, height: 560 }}>
          <div className="w-[560px] h-[560px]">
            <Chessboard
              ...
              arePiecesDraggable={!isOver && !!meSide && turn === meSide && !selectingShield && !selectingCounterSac}
              isDraggablePiece={({ piece }) => {
                if (!meSide || turn !== meSide || selectingShield || selectingCounterSac) return false;
              ...
              customSquareStyles={{
                ...(shield.square
                  ? { [shield.square]: { boxShadow: 'inset 0 0 0 3px rgba(56,189,248,0.85)' } }
                  : {}),
              }}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {hand.map((c) => (
              <div key={c.uid} className="bg-gray-800 p-3 rounded-xl">
                ...
                <button
                  onClick={() => playCard(c)}
                  disabled={isOver || !meSide || turn !== meSide || cardPlayedBy === meSide}
        ...
        <div className="text-sm opacity-80 space-y-1 flex-shrink-0">
          <div>Extra move เหลือ: W {extraMove.w} / B {extraMove.b}</div>
          <div>Shield: {shield.square ? `${shield.square} (โดย ${shield.by==='w'?'white':'black'})` : '-'}</div>
          <div className="flex items-center gap-2">
            <input className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-2 py-1" value={inviteUrl} readOnly />
            <button
        ...
        {(selectingShield || selectingCounterSac) && (
          <div className="text-center text-sm -mt-2">
            {selectingShield
              ? <span className="text-cyan-300">โหมดโล่ป้องกัน: คลิกหมากของตัวเองเพื่อคุ้มกัน</span>
              : <span className="text-amber-300">โต้กลับ: คลิก “หมากของคุณ (ยกเว้นราชา)” 1 ตัวเพื่อสละชีพแทน</span>}
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-900 rounded-2xl p-4 text-white flex flex-col h-[560px]">
        ...
        <div className="flex-1 overflow-y-auto mt-2 space-y-1">
          {chat.map((line, i) => (
            <div key={i} className="text-sm bg-gray-800 px-2 py-1 rounded">{line}</div>
          ))}
        </div>
        <div className="mt-2 flex gap-2">
          ...
          />
          <button onClick={sendChat} className="px-3 py-1.5 rounded bg-indigo-500 hover:bg-indigo-600">
            ส่ง
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-xl rounded-2xl bg-gray-900 border border-gray-700 shadow-xl p-4">
            <div className="text-center mb-3">
              ...
              {result?.type === 'checkmate' && result?.winner && (
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded text-xs ${restartVotes.has('w') ? 'bg-emerald-600' : 'bg-gray-700'}`}>
                    White {restartVotes.has('w') ? '✔' : '…'}
                  </span>
                  <span className={`px-2 py-1 rounded text-xs ${restartVotes.has('b') ? 'bg-emerald-600' : 'bg-gray-700'}`}>
              ...
              <div className="flex items-center gap-2">
                {restartCounting ? (
                  ...
                  >
                    {(!meSide || !restartVotes.has(meSide)) ? 'พร้อมเริ่มใหม่' : 'รออีกฝั่ง…'}
                  </button>
                )}
              </div>
            </div>
